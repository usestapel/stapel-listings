"""stapel-listings contract-emission harness (contract-pipeline.md §2-3).
...
stapel-listings is not mounted in stapel-example-monolith, so there is no
aggregate slice to diff against for byte-identity — validation is standalone
(determinism + closure + canonical prefix), see ``tests/test_contract.py``.

Usage:
    python -m stapel_listings._codegen --out docs        # `make contract`
"""
...
def _configure() -> None:
    ...
    # flat-layout conftest does, so `import listings`-shaped collisions
    ...
    if not settings.configured:
        from stapel_listings._codegen_settings import settings_kwargs

        settings.configure(
            **settings_kwargs(root_urlconf="stapel_listings.codegen_urls", contract=True)
        )
    ...
    # but "/listings/api" in a single-module harness, which would strip it to
    # bare anonymous names. Pin it to the aggregate convention.
    from drf_spectacular.settings import spectacular_settings

    from stapel_listings._codegen_settings import CODEGEN_SCHEMA_PATH_PREFIX
...
def _require_python_312() -> None:
    ...
    if sys.version_info[:2] != (3, 12):
        ...
        raise SystemExit(
            f"stapel-listings contract emission ABORTED: running Python {got}, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="stapel-listings-contract",
        description="Emit this module's contract triad (schema.json + flows.json "
        "+ errors.json) into --out, canonical /listings/api/v1 prefix.",
    )
    ...
    print(
        f"stapel-listings contract: {paths} paths, {flows} flows, {errors} error keys "
        f"-> {out}/",
        file=sys.stderr,
    )
    return 0
