...
class TestLazyExports:
    def test_all_declares_public_api(self):
        assert stapel_listings.__all__ == [
            "listings_settings",
        ]
    ...
    def test_settings_resolve(self):
        ...
        assert stapel_listings.listings_settings is listings_settings

    def test_unknown_attribute_raises(self):
        try:
            stapel_listings.nonexistent_export
...
class TestImportWithoutDjangoSettings:
    def test_package_import_is_django_free(self):
        """`import stapel_listings` must not import Django nor require settings."""
